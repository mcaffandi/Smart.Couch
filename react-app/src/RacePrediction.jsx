...
function detectStagnation(activities) {
  const now = Date.now();
...
function GoalTimeInput({ value, onChange }) {
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
      {[
        { key: 'h', label: 'jam', max: 9 },
        { key: 'm', label: 'menit', max: 59 },
        { key: 's', label: 'detik', max: 59 },
      ].map(({ key, label, max }) => (
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function RacePrediction({ activities, targetPace }) {
  ...
  const stagnation = useMemo(() => detectStagnation(activities), [activities]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>
            Data Terbaik — {dataLabel}
          </div>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>
            {refRun.distKm.toFixed(1)} km &nbsp;·&nbsp; {formatPace(refRun.paceMinKm)} min/km
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>Sesi tercepat lo sebagai referensi kalkulasi</div>
        </div>
        <div style={{ display: 'flex', gap: 24 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Akurasi</div>
            <div style={{ fontWeight: 800, fontSize: 18, color: confColor, marginTop: 2 }}>{confidence}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{(recentRuns.length || allRuns.length)} sesi valid</div>
          </div>
          {vo2max > 0 && (
            <div style={{ textAlign: 'center' }}>
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)', marginBottom: 4 }}>
          Estimasi Waktu Mencapai Target
        </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>
            Target Waktu untuk {targetRace?.label}
            <span style={{ color: '#fb7185', marginLeft: 4 }}>*</span>
          </div>
          <GoalTimeInput value={goalTime} onChange={setGoalTime} />
        </div>
        ...
        {!goalValid ? (
          <div style={{
            ...
            Isi target waktu di atas untuk melihat estimasi
          </div>
        ) : alreadyAchieved ? (
          <div className="alert alert-success">
            <strong>Target sudah tercapai!</strong> Prediksi current lo ({secsToTime(currentPred.predSec)}) sudah lebih cepat dari target {secsToTime(goalSecs)}.
            Naikkan target lo atau pilih distance lebih jauh!
          </div>
        ) : (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20, flexWrap: 'wrap', marginBottom: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 160 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Target Lo</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: '#818cf8', marginTop: 2 }}>{secsToTime(goalSecs)}</div>
                  </div>
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Pace Target</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)', marginTop: 2 }}>{formatPace(goalPaceMinKm)} /km</div>
                  </div>
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Perlu Improve</div>
            ...
            <div style={{ marginTop: 4 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 6 }}>
                <span>Pace sekarang: {formatPace(refRun.paceMinKm)}</span>
                <span>Target: {formatPace(goalPaceMinKm)}</span>
              </div>
      ...
      {vo2max > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 20px' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 10 }}>
            Estimasi VO2Max: <span style={{ color: '#818cf8' }}>{vo2max}</span> ml/kg/min
          </div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            {vo2max < 30 ? 'Pemula — baseline yang bagus. Lari konsisten 3×/minggu bisa naik 5–8 poin dalam 3 bulan pertama.' :
             vo2max < 38 ? 'Di bawah rata-rata — potensi improve besar. Fokus easy run volume dulu sebelum interval.' :
             vo2max < 46 ? 'Rata-rata — level yang solid. Tambahkan interval 1×/minggu untuk naik ke level berikutnya.' :
             vo2max < 52 ? 'Di atas rata-rata — atletis. Tempo run + progressive overload bisa push lebih jauh.' :
             vo2max < 57 ? 'Baik sekali — setara pelari kompetitif amatir. Jaga volume dan hindari overtraining.' :
             vo2max < 62 ? 'Sangat baik — top 10% populasi. Prioritaskan recovery dan peak week periodization.' :
             'Level elite — di sini butuh coach spesialis dan program periodization ketat.'}
          </div>
        </div>
      )}
